...
如果仓库开启了 GitHub Pages，中文展示页位于 [`docs/index.html`](./docs/index.html)，英文展示页位于 [`docs/en.html`](./docs/en.html)。

展示页和截图使用匿名示例数据，不包含个人保存链接。
...
- 没有提交真实 Webhook URL。
- 没有提交个人浏览记录或导出的 `chrome.storage` 数据。
- 截图使用匿名或虚构示例数据。
- 如需发布新版，更新 `manifest.json` 版本号。
- 如需 GitHub Releases，可以打包扩展 zip。

### 项目状态

LaterNest 目前是一个可用的个人 MVP，并刻意保持克制：
...
If GitHub Pages is enabled for this repository, the Chinese product page lives in [`docs/index.html`](./docs/index.html), and the English page lives in [`docs/en.html`](./docs/en.html).

The demo page and screenshots use fictional sample data and do not contain personal saved links.
